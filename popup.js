    ...
    chrome.storage.sync.get(['notionToken', 'databaseId'], (result) => {
      ...
      chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
          ...
          showStatusMessage(
            response.error || 'Failed to save job to Notion. Please check your settings.',
            'error'
          );
        }
      });
      ...
      chrome.tabs.sendMessage(
        ...
        (response) => {
          ...
          if (chrome.runtime.lastError) {
            ...
            if (jobSite) {
              showStatusMessage(`This appears to be a ${jobSite} job listing. Please fill in the details manually.`, '');
            }
            return;
          }
          ...
          if (response) {
            ...
            if (response.company || response.position || response.location || response.salary || response.description) {
              showStatusMessage('Job information extracted successfully!', 'success');
              setTimeout(() => {
                statusMessage.textContent = '';
                statusMessage.className = '';
              }, 3000);
            } else {
              showStatusMessage('Could not automatically extract job details. Please fill in manually.', '');
            }
          }
        }
      );
